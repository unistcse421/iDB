...
//       .tickFormat(function(d) { return "e" + formatPower(Math.round(Math.log(d))); });
  ...
  svg1.append("path")
      ...
      .attr("class", "line")
      .attr("d", line)
      .on("mouseover", function(d,i) { svg.append("g").attr("class","date").attr("x", x(i)).attr("y", height-d["value"]-5).text(d["date"]); })
...
//     y.domain([0, d3.max(data, function(d) { return d.frequency; })]);
    ...
    svg.selectAll(".bar")
      ...
      .enter().append("rect")
        ...
        .on("mouseover", function(d,i) { svg.append("g").attr("class","date").attr("x", x(i)).attr("y", height-tData[i]["value"]-5).text(tData[i]["date"]); })
        .on("mouseout", function(d,i) { svg.selectAll(".date").remove(); });
